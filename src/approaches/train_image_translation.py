...
#                   writer.write(frame.astype(np.uint8)) #関係ない
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        self.G.eval()
        os.system('echo')           
        os.system('ls examples/*.mp4')
        os.system('echo')           
        print('1111111111  66666666666666')
        ...
        print('1111111111  77777777777777')
        writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 1, 256))
#       writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 2, 256))
#       writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
#       writer2 = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
        print('1111111111  88888888888888')
        os.system('echo')           
        os.system('ls examples/*.mp4')
        os.system('echo')           
...
#               frame = np.concatenate(g_out[i]) * 255.0
#               frame=g_out[i]*255.0
                frame = np.concatenate((g_out), axis=1) * 255.0
#               frame = np.concatenate((ref_in[i], g_out[i]), axis=1) * 255.0
#               frame = np.concatenate((ref_in[i], g_out[i], fls_in[i]), axis=1) * 255.0
#               writer.write(frame.astype(np.uint8))
                writer.write(frame.astype(np.uint8)) ### <-------ここが重要:
