...
import torch, pickle, face_alignment, shutil, time
#mport pickle
...
def Say00(data):
    tts=gTTS(data)
    tts.save('answer.wav')
    shutil.copyfile("answer.wav", "./examples/answer.wav")
#   playsound('answer.wav')

''' STEP 1 -----------------'''
default_head_name = 'kato2.png'
ADD_NAIVE_EYE = True
...
#img =cv2.imread('examples/e3sss.png')
img =cv2.imread('examples/kato2.png')
...
#################################################################################################
''' STEP 2: RUN audio->landmark network'''
...
#################################################################################################
        ...
        ''' STEP 3: de-normalize the output to the original image scale '''
        ...
        clip=VideoFileClip('examples/kato2_pred_fls_answer_audio_embed.mp4')
        clip=clip.resize(6)

        timeDiff=int(time.time()-t01)
        timeDiff2=str(timeDiff) 
        print('timeDiff=',timeDiff)

        conn.send(timeDiff2.encode())  #usami
        conn.close()
